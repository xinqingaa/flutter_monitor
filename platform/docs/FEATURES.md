...
一级导航（侧栏展开仅文案；收起为图标，hover 出文案；收起控件在侧栏底部）：
...
- 按 sessionId 切换会话；列表进入不预选事件，默认全部段收起；列表上方左侧展开/收起全部图标、右侧 `N 段`；
- 顶栏身份摘要与异常/事件/时长同一行；
- 全宽时间链（启动/页面分段、短路由段头、两行上下文、Trace 色轨；段展开有过渡）；
- 全部 / 启动 / 页面 / HTTP / 埋点 / 异常分类（失败 HTTP 不进异常；切回全部会清除 URL `tab`）；
...
- 右侧 scrubber：始终按当前视图可见段刻度（统一淡灰刻度 + tooltip）；
- 行菜单含「仅看此 Trace」；域列表快捷入口仅对 HTTP / 埋点 / 异常；
