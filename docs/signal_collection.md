...
- `target` -> `ui.target`
- 调用时 route context（或显式 `routeName`/`routeFullName`）-> envelope `context.route.*`
- 调用时页面 trace（显式路由命中已有 page trace 时复用其 trace）-> envelope `traceId`
- 调用时页面实例（显式路由命中已有 page trace 时复用）-> `page.instance_id`
- `result` -> `business.result`、`status`
- `properties` -> payload 中的业务详情
- `observeFor` -> `interaction.observe_ms`
- `timeout` -> `interaction.timeout_ms`
- `routeName` / `routeFullName`（可选）-> 覆盖归属路由，写入 `context.route.*`；用于自动栈顶路由不准的场景，取值必须是稳定低基数路由，不得含动态业务 ID
